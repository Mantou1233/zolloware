...
export default class Help extends Command<[string]> {
	...
	public getMessageForAllTypes(source: Source): MessageCreateOptions {
		return {
			components: this.getComponentsForAllTypes(source),
			embeds: this.getEmbedsForAllTypes(source)
		};
	}

	public getComponentsForAllTypes(source: Source): ActionRowBuilder<ButtonBuilder | SelectMenuBuilder>[] {
		const menu = new SelectMenuBuilder().setCustomId("help_menu_main").setPlaceholder("請選擇一個指令分類");

		for (const type of Object.values(CommandType)
			.filter((t): t is CommandType => typeof t === "number")
			.filter(t => source.client.commands.commands.filter((c: Command) => c.type === t))) {
	...
	public getMessageForType(interaction: SelectMenuInteraction<"cached">, type: CommandType): InteractionReplyOptions | void {
		const components = this.getComponentsForType(interaction, type);
		const embeds = this.getEmbedsForType(interaction, type);
	...
	public getComponentsForType(interaction: SelectMenuInteraction<"cached">, type: CommandType): ActionRowBuilder<ButtonBuilder | SelectMenuBuilder>[] | void {
		const menu = new SelectMenuBuilder().setCustomId("help_menu_type").setPlaceholder("請選擇一個指令");
		const commands =
	...
	private getDescriptionForCommand(command: Command, isSubcommand?: boolean): string {
		...
		if (command.alias) description += `** - 替代名稱：**${command.alias.map(a => `\`${a}\``).join(", ")}\n`;
	...
	private optionsToString(options: ExtendedCommandOptionData[]): string {
		...
		for (const option of options) {
			...
			if ("minValue" in option || "maxValue" in option) {
				description += `　- 數值範圍：\`${option.minValue !== undefined ? `[${option.minValue}` : "(-∞"}, ${
					option.maxValue !== undefined ? `${option.maxValue}]` : "∞)"
				}\``;
			}
			if ("minLength" in option || "maxLength" in option) {
				description += `　- 長度範圍：\`${option.minLength !== undefined ? `[${option.minLength}` : "(0"}, ${
					option.maxLength !== undefined ? `${option.maxLength}]` : "∞)"
				}\``;
			}
		}
		return description;
