...
export class HZClient extends Client {
	...
	constructor(options: HZClientOptions) {
		...
		this.bugHook = new WebhookClient({
			...
			token: config.webhooks.bug.token
		});
		this.suggestHook = new WebhookClient({
			id: config.webhooks.suggest.id,
			token: config.webhooks.suggest.token
		});
		this.replyHook = new WebhookClient({
			id: config.webhooks.reply.id,
			token: config.webhooks.reply.token
		});
	}
		...
		switch (command) {
			...
			case "network":
				return message.channel.send(
					`若 Network 沒有正常同步，請先確認你的頻道名稱。如果還是不行，請檢查 HiZollo 有沒有 __${Translator.getPermissionChinese(
						"ManageWebhooks"
					)}__ 的權限`
				);
			...
			case "help":
				return message.channel.send(
					`請使用 \`${config.bot.prefix}help [指令名稱]\` 查詢一個指令的使用方法，裡面都有說明、用法、範例\n若還是不懂，請說明哪裡看不懂`
				);
