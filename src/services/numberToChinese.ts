function intToChinese(num: number, digits: string[], units: any[], minus: any, isCheque?: boolean) {
	let str = "";
...
function floatToChinese(num: number, digits: string[], point: string) {
	if (num % 1 == 0) return "";
...
class NC {
	...
	static toOrdinal(num: any) {
		return NC.labels.ordinal + num;
	}

	static toWords(num: any) {
