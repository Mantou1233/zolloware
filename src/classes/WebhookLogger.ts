...
export class WebhookLogger {
	...
	constructor(client: ExtendedClient) {
		...
		this.mainLogger = new WebhookClient({
			id: config.webhooks.main.id,
			token: config.webhooks.main.token
		});
		this.networkLogger = new WebhookClient({
			id: config.webhooks.network.id,
			token: config.webhooks.network.token
		});
		this.errorLogger = new WebhookClient({
			id: config.webhooks.error.id,
			token: config.webhooks.error.token
		});
	}
	 ...
	 */
	public ready(): void {
		const description = `成功登入 ${this.client.user?.tag}
...
本分支目前服務 ${this.client.guilds.cache.size} 個伺服器`;
	...
	public async joinGuild(guild: Guild): Promise<void> {
		const description = `加入伺服器：${guild.name}
ID：${guild.id}
目前服務 ${await this.client.guildCount()} 個伺服器`;
	...
	public async leaveGuild(guild: Guild): Promise<void> {
		const description = `離開伺服器：${guild.name}
ID：${guild.id}
目前服務 ${await this.client.guildCount()} 個伺服器`;
	...
	public error(error: Error): void {
		...
		log.setDescription(`**${ename}**\n${estacks.join("\n")}`).setFooter({
			text: `分支編號：${this.client.shard?.ids[0]}`
		});
...
伺服器：${message.guild!.id}`;
	...
	public networkCrossPost(portNo: string, guild: Guild, author: User): void {
		const description = `在 ${portNo} 號埠上發送訊息
伺服器：${guild.id}
使用者：${author.tag}（${author.id}）`;
	...
	public networkBroadcast(portNo: string, content: string): void {
		const description = `在 ${portNo} 號埠上全頻廣播
廣播內容：${content}`;
	...
	public networkJoined(portNo: string, channel: TextChannel): void {
		const description = `在 ${portNo} 號埠上建立連線
...
頻道：${channel.id}`;
	...
	public networkLeft(portNo: string, channel: TextChannel): void {
		const description = `在 ${portNo} 號埠上刪除連線
伺服器：${channel.guild.id}
頻道：${channel.id}`;
