...
export class CommandManager extends EventEmitter {
		...
		for (const file of commandFiles) {
			...
			if (instance.type === CommandType.SubcommandGroup) {
				this.subcommands.load(
					path.join(dirPath, file.slice(0, -3)),
					instance
				);
			} else {
	...
	public async onInteractionCreate(interaction: Interaction): Promise<void> {
		...
		if (this.client.devMode && !interaction.channel?.isTestChannel())
			return;
		...
		const commandName: [string, string | undefined] = [
		...
		if (command.type === CommandType.Developer && !channel.isTestChannel())
			return;
		...
		const userMissing = missingPermissions(
		...
		if (!channel.isTestChannel()) {
			const timeLeft = await this.client.cooldown.checkUser({
				commandName: command.name,
				userId: interaction.user.id
			});
			if (timeLeft) {
				this.emit("reject", new Source(interaction, channel, member), {
					reason: CommandManagerRejectReason.InCooldown,
					args: [timeLeft]
				});
				return;
			}
		...
		if (result.status !== CommandParserOptionResultStatus.Pass) {
			this.emit("reject", new Source(interaction, channel, member), {
				...
				args: [commandName, command.options ?? [], result]
			});
			return;
		}
		...
		/***** 執行 *****/
		try {
			const source = await command.slashExecute(
				interaction,
				args,
				channel,
				member
	...
	public async onMessageCreate(message: Message): Promise<void> {
		...
		if (command instanceof Command) {
			commandName = command.parent
				? [command.parent, command.name]
				: [command.name, undefined];

			const matchingNames = [command.name, ...(command.aliases ?? [])];
			if (
				!matchingNames.includes(firstArg) &&
				matchingNames.includes(secondArg)
			) {
		...
		if (
			command.twoFactorRequired &&
			message.guild.mfaLevel === GuildMFALevel.Elevated
		) {
			this.emit("reject", new Source(message, channel, member), {
				reason: CommandManagerRejectReason.TwoFactorRequird,
				args: []
		...
		if (!message.channel.isTestChannel()) {
			const timeLeft = await message.client.cooldown.checkUser({
				commandName: command.name,
		...
		/***** 檢查參數 *****/
		const result = await CommandParser.parseMessageArgs(
			message,
			rawArgs,
			command
		);
		...
		/***** 執行 *****/
		try {
			const source = await command.messageExecute(
				message,
				args,
				channel,
				member
	...
	public on(
		...
		listener: (
			source: Source,
			info: CommandManagerRejectInfo
		) => Awaitable<void>
	): this;
	public on<K extends keyof CommandManagerEvents>(
		event: K,
		listener: (...args: CommandManagerEvents[K]) => Awaitable<void>
	): this;
	public on<S extends string | symbol>(
		event: Exclude<S, keyof CommandManagerEvents | "reject">,
	...
	public once<K extends keyof CommandManagerEvents>(
		event: K,
		listener: (...args: CommandManagerEvents[K]) => Awaitable<void>
	): this;
	...
	public emit<K extends keyof CommandManagerEvents>(
		event: K,
		...args: CommandManagerEvents[K]
	): boolean;
	...
	public off<K extends keyof CommandManagerEvents>(
		event: K,
		listener: (...args: CommandManagerEvents[K]) => Awaitable<void>
	): this;
